...
from cfg import IS_RGBD, NUM_CH
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        model = Model(opt.cfg or ckpt['model'].yaml, ch=NUM_CH, nc=nc).to(device)  # create
        ...
        model = Model(opt.cfg, ch=NUM_CH, nc=nc).to(device)# create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            print(('\n' + '%10s' * 10) % ('Epoch', 'gpu_mem', 'dist', 'obj', 'cls', 'prob', 'free', 'total', 'targets', 'img_size'))
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                # Plot
                if ni < 3:
                    f = str(log_dir / ('epoch%gtrain_batch%ggt.jpg' % (epoch, ni)))  # filename
                    result = plot_images(images=imgs, targets=targets, paths=paths, fname=f, ch=NUM_CH)
                    if tb_writer and result is not None:
                        tb_writer.add_image(f, result, dataformats='HWC', global_step=epoch)

                # additional image output during first part of first epoch (for ease of debugging)
                if (epoch==start_epoch and 0<i<=2000 and i%500==0) or (start_epoch>0 and epoch==start_epoch and i==0):
                    if ema is not None:
        ...
        if rank in [-1, 0]:
            ...
            # Tensorboard
            if tb_writer:
                tags = ['train/giou_loss', 'train/obj_loss', 'train/cls_loss', 'train/prob_loss'
